...
impl Solution {
    pub fn int_to_roman(num: i32) -> String {
        let mut num = num;
        let mut len = Self::get_digits(num);
        ...
        while num > 0 {
            ...
            while val > 0 {
                if [4, 9].contains(&start) {
                    let (v, new_val) = Self::get_diff_49(val);
                    val = new_val;
                    loc.push(Self::map_value(v).unwrap());

                    if let Some(ch) = Self::map_value(val) {
                        loc.insert(0, ch);
                        break;
                    }
                } else {
                    let (v, new_val) = Self::get_diff(val);
                    val = new_val;
                    loc.push(Self::map_value(v).unwrap());
                ...
                start = val / 10_i32.pow(Self::get_digits(val))
            }
